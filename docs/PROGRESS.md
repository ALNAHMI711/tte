# TTE — حالة المشروع

## الحالة الحالية
- المرحلة: Core + Security + Market + Paper Execution foundation
- الفرع: `foundation-v1`
- Live Trading: 🔴 مقفول
- Paper Trading: 🟢 مفعّل كمسار الاختبار الأول
- أسرار/API keys: 🔒 لا تُحفظ في Git
- آخر دفعة: إضافة جلسات آمنة + رفع صلاحية الإدارة المؤقتة + حماية محاولات الدخول

## لوحة المتابعة
| المجال | الحالة |
|---|---:|
| الأساس البرمجي | 43% |
| الأمان وإدارة الأسرار | 42% |
| لوحة التحكم RTL | 10% |
| المحافظ والمنصات | 0% |
| محرك السوق | 15% |
| الاستراتيجيات | 0% |
| إدارة المخاطر | 25% |
| التنفيذ | 20% |
| Paper Trading | 25% |
| AI Decision Layer | 0% |
| GitHub Strategy Updates | 0% |
| الاختبارات | 32% |
| التوثيق | 15% |

**التقدم الإجمالي التقريبي: 23%**

> النسبة مؤشر سير عمل وليست نسبة ضمان نجاح أو ربح.

## ما تم إنجازه
- `pyproject.toml` لتثبيت الحزمة واكتشاف `trading*` بشكل صريح.
- Argon2id لتجزئة كلمات المرور مع حد أدنى 12 حرفاً واختبارات تحقق.
- جلسات opaque عشوائية، انتهاء صلاحية، إلغاء جلسة، وStep-up مؤقت مرتبط بالجلسة.
- حماية أولية لمحاولات تسجيل الدخول عبر exponential backoff وقفل مؤقت بعد عدة إخفاقات.
- نموذج Audit Event غير قابل للتعديل لتسجيل الأحداث الأمنية دون وضع الأسرار داخل السجل.
- CI يثبت المشروع واعتمادياته ثم compile + pytest.
- Live Trading ما زال مقفولاً افتراضياً ولا يمكن فتحه من هذه الطبقة الأساسية.

## بوابة الإطلاق
لا يتم فتح Live Trading حتى تنجح اختبارات الأمان، المصادقة، الأسرار، البيانات، المخاطر، التنفيذ، Paper Trading، وفحوصات CI بدون أخطاء حرجة، ثم اجتياز فحوص Binance Trusted IP والصلاحيات.

## الخطوات التالية ذات الأولوية
1. ربط المصادقة والجلسات فعلياً بطبقة HTTP/API مع Secure + HttpOnly + SameSite cookies وCSRF.
2. إضافة تخزين أسرار مشفّر/Secret Provider مع masking واختبارات عدم التسريب.
3. بناء health/readiness وAudit persistence.
4. بناء عقود Exchange Adapter وMarket Data ثم Binance Testnet.
5. توسيع Risk Engine إلى حدود المستخدم المطلوبة: 0.5%/صفقة، خسارة يومية/أسبوعية، التعرض، correlation وkill switch.
6. استراتيجية/Backtest/Paper reconciliation ثم لوحة RTL.
7. فحوصات أمنية CI مثل dependency audit وsecret scanning وstatic analysis.

## خريطة لوحة التحكم
1. **الأمان:** كلمة مرور الدخول + تحقق إضافي للإدارة الحساسة.
2. **المنصات:** اسم المنصة، API Key، API Secret، Testnet/Live، اختبار الاتصال، الصلاحيات.
3. **المحافظ:** عنوان/معرّف المحفظة حسب نوع التكامل؛ لا يتم إدخال seed phrase أو private key في الواجهة.
4. **السوق:** الزوج، الإطار الزمني، OHLCV، الحجم، السيولة، السبريد، دفتر الأوامر.
5. **الاستراتيجيات:** اختيار/إضافة/نسخة/Backtest/Paper Test.
6. **AI:** الإشارات، التحليل، الثقة، التفسير؛ لا يتجاوز محرك المخاطر.
7. **المخاطر:** حجم الصفقة، SL/TP، التعرض، الخسارة اليومية، الانزلاق، circuit breaker.
8. **التنفيذ:** الأوامر، الحالات، partial fills، idempotency.
9. **التحكم:** تشغيل، إيقاف، إيقاف طوارئ.
10. **السجل:** Audit Log لكل تغيير وقرار وأمر.
